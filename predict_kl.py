...
def generate_predictions_excl_deltas(machine_class, machine_id, test_dir="test", small=False):
    if small == True:
        h5_train = h5py.File(param['feature_root'] + '/' + '_'.join([machine_class, machine_id, 'train', 'mfcc_small']) + '.hdf5', 'r')
    else:
        h5_train = h5py.File(param['feature_root'] + '/' + '_'.join([machine_class, machine_id, 'train', 'mfcc2']) + '.hdf5', 'r')
